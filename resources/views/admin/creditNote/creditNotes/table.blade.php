...
<table class="table table-striped table-bordered" id="creditNotes-table" width="100%">
    <thead>
     <tr>
    ...
    @foreach($creditNotes as $creditNote)
        <tr>
            ...
            @if (Sentinel::inRole('admin'))
                 ...
                 <a href="{{ route('admin.creditNote.creditNotes.confirm-delete', collect($creditNote)->first() ) }}" data-toggle="modal" data-target="#delete_confirm" data-id="{{ route('admin.creditNote.creditNotes.delete', collect($creditNote)->first() ) }}">
 ...
 <script type="text/javascript" src="{{ asset('vendors/datatables/js/dataTables.bootstrap4.js') }}" ></script>
    ...
    <script>
        $('#creditNotes-table').DataTable({
                  ...
                  $('#creditNotes-table').on( 'page.dt', function () {
                     setTimeout(function(){
                           $('.livicon').updateLivicon();
                     },500);
                  } );
                  $('#creditNotes-table').on( 'length.dt', function ( e, settings, len ) {
                     setTimeout(function(){
                            $('.livicon').updateLivicon();
                     },500);
                  } );
