    ...
    @foreach($invoicwePayments as $invoicwePayment)
        <tr>
            ...
            @if (Sentinel::inRole('admin'))
                 ...
                 <a href="{{ route('admin.invoicwePayment.invoicwePayments.confirm-delete', collect($invoicwePayment)->first() ) }}" data-toggle="modal" data-target="#delete_confirm" data-id="{{ route('admin.invoicwePayment.invoicwePayments.delete', collect($invoicwePayment)->first() ) }}">
 ...
 <script type="text/javascript" src="{{ asset('vendors/datatables/js/dataTables.bootstrap4.js') }}" ></script>
    ...
    <script>
        $('#invoicwePayments-table').DataTable({
                  ...
                  $('#invoicwePayments-table').on( 'page.dt', function () {
                     setTimeout(function(){
                           $('.livicon').updateLivicon();
                     },500);
                  } );
                  $('#invoicwePayments-table').on( 'length.dt', function ( e, settings, len ) {
                     setTimeout(function(){
                            $('.livicon').updateLivicon();
                     },500);
                  } );
