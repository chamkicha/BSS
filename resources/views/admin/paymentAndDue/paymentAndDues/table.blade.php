    ...
    @foreach($paymentAndDues as $paymentAndDue)
        <tr>
            ...
            @if (Sentinel::inRole('admin'))
                 ...
                 <a href="{{ route('admin.paymentAndDue.paymentAndDues.confirm-delete', collect($paymentAndDue)->first() ) }}" data-toggle="modal" data-target="#delete_confirm" data-id="{{ route('admin.paymentAndDue.paymentAndDues.delete', collect($paymentAndDue)->first() ) }}">
 ...
 <script type="text/javascript" src="{{ asset('vendors/datatables/js/dataTables.bootstrap4.js') }}" ></script>
    ...
    <script>
        $('#paymentAndDues-table').DataTable({
                  ...
                  $('#paymentAndDues-table').on( 'page.dt', function () {
                     setTimeout(function(){
                           $('.livicon').updateLivicon();
                     },500);
                  } );
                  $('#paymentAndDues-table').on( 'length.dt', function ( e, settings, len ) {
                     setTimeout(function(){
                            $('.livicon').updateLivicon();
                     },500);
                  } );
